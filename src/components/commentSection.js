...
function CommentSection({ props }) {
			...
			await addPost({
				...
				getPosts({
					...
					if (res.Empty) {
						...
						const sortedArray = result.sort((a, b) => new Moment(b[1].time_stamp) - new Moment(a[1].time_stamp));
						setEmpty("");
	...
	function SortArray() {
		...
		const sortedArray = messages.data.sort((a, b) => new Moment(b[1].time_stamp) - new Moment(a[1].time_stamp));
	}

	useEffect(() => {
		console.log("Works");
		console.log("test1234", props.location);
		...
		<div className="commentSection">
			...
			<div className="containerAddComment">
				...
				<div className="addComment">
					...
					<div>
						<label>Char Name</label> <input onChange={handleNameChange} placeholder="Your char name" required></input>
					</div>

					<div>
						<label>Comment</label>{" "}
						<textarea onChange={handleMessageChange} placeholder="Add a public comment" required></textarea>
					</div>
			...
			{empty ? <h1 className="empty">{empty}</h1> : null}
			{messages.loading ? (
											...
											message[0] === comments2[1].id_reply && (
												<div key={comments2[0]}>
													<div className="containerCommentReply">
														<img
															src={
																(comments2[1].icon === "profileIcon1" && IconProfile1) ||
																(comments2[1].icon === "profileIcon2" && IconProfile2) ||
																(comments2[1].icon === "profileIcon3" && IconProfile3)
															}
														></img>

														<div className="actualCommentReply">
															<div className="charName"> {comments2[1].charName}</div>
															<div className="message">{comments2[1].message} </div>
															<div className="time">
