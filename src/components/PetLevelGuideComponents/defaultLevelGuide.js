        ...
        <div className="defaultLevelGuide">
            ...
            <div className="containerLevelGuide">
                <div className="containerLeft">
                    ...
                    <p>
                        The most effective way to level pets is via Pet Trainers because they grant 4-6 times more XP than an usual wild pet. Sadly most of them are having a daily lockout, being
                        doable once per day, that's why I suggest you leaving one of your alts in the desired spot of each pet masters.
                    </p>
                </div>
                <div className="containerRight">
                    <h3>Pets Required for the Fights</h3>
                    <p>
                        "The pets I recommended for the fights are taken from your collection. Usually there are 1-3 pets suggested for each slot and just the best is displayed (from the ones you
                        own)."
                    </p>
                </div>
            </div>
                ...
                {!requiredPets.loading ? (
                    requiredPets.pets.map((pet, index) => (
                        <li key={index} className="defaultPet">
                            <a href={"https://www.wowhead.com/battle-pet/" + pet.id} target="_blank">
                                <img src={pet.icon}></img>{" "}
                            </a>
                            <span className={checkOwnedPet(pet.id) ? "owned" : "notOwned"}>{pet.name} </span>
                            {checkOwnedPet(pet.id) ? <FontAwesomeIcon className="owned" icon={faCheck} /> : <FontAwesomeIcon className="notOwned" icon={faTimes} />}
