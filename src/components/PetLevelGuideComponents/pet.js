...
import { faSkull, faPaw, faFeatherAlt, faFrog, faCogs, faCog, faMask, faCheck, faTimes } from "@fortawesome/free-solid-svg-icons";
...
function ShadowmoonDraenor({ pet, petOwned }) {
    ...
    function checkPetLevel25(id) {
        ...
        if (level25Pets.length > 0) setPetOwnedDetails({ loading: false, petDetails: [level25Pets[0]] });
    ...
    return (
        <li className={[checkType(pet.pets.battle_pet_type.type)]} style={{ animationDuration: 0.8 + "s" }}>
            <div className="iconPet">
                <a href={"https://www.wowhead.com/battle-pet/" + pet.pets.id} target="_blank">
                ...
                {petOwnedDetails.petDetails && !petOwnedDetails.loading && ownedPet ? (
                    petOwnedDetails.petDetails.map((detail) => (
                        <div>
                            ...
                            {breed === "S" ? (
                                detail.breed === 21 || detail.breed === 11 || detail.breed === 9 || detail.breed === 19 || detail.breed === 8 || detail.breed === 18 ? (
                    ...
                    <div>
                        ...
                        {speed === null ? <p className="stats">No Stats Required </p> : <p className="stats required">Required Speed {speed})</p>}
            ...
            {petAbilitiesState != null && petAbilitiesState.length === 6 ? (
                <div className="abilities">
                    <div>
                        <img src={petAbilitiesState[abilities[1]].assets[0].value} alt="noAbility"></img>
                        {pet.pets.abilities[abilities[1]].ability.name}
                    </div>
                    <div>
                        <img src={petAbilitiesState[abilities[2]].assets[0].value} alt="noAbility"></img>
                        {pet.pets.abilities[abilities[2]].ability.name}
                    </div>
                    <div>
                        <img src={petAbilitiesState[abilities[3]].assets[0].value} alt="noAbility"></img>
            ...
            {petsChar.pets.pets.length > 0 ? (
                <div>
                    <p className={ownedPet ? "owned" : "notOwned"}>{ownedPet ? "Owned" : "Not owned"}</p>
                    {ownedPet ? <FontAwesomeIcon className="check" icon={faCheck} /> : <FontAwesomeIcon className="notChecked" icon={faTimes} />}
                </div>
            ) : (
            ...
            {pet.pets.battle_pet_type.type === "BEAST" ? <div className="swipe"></div> : ""}
            {pet.pets.battle_pet_type.type === "BEAST" ? <FontAwesomeIcon className="paw" icon={faPaw} /> : ""}
            {pet.pets.battle_pet_type.type === "ELEMENTAL" ? <div className="fireIcon"></div> : ""}
            {pet.pets.battle_pet_type.type === "UNDEAD" ? (
                <div>
                    <FontAwesomeIcon className="skull" icon={faSkull} style={{ top: 3 * 7 + "%" }} />
