...
function MaldraxxusDefault() {
  ...
  useMemo(() => {
    ...
    petInfo2().then((res) => {
      ...
      petsInfo(idPets, idPets.length).then((res3) => {
        res3.map((res4) => {
          if (res4.creature.name === "Rotten Little Helper")
            console.log("sentry " + res4.id);
        });
      });
    });
    */
  ...
  return (
    <div className="maldraxxusDefault">
      <h1>Maldraxxus Pet World Quests Guides</h1>
      <h2>Choose a guide to follow up from the menu on the left!</h2>
      <p>
        Maldraxxus, realm of Necrolords and location of four of the Pet Battle
...
export default MaldraxxusDefault;
